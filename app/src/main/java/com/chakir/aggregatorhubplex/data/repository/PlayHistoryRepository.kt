...
interface PlayHistoryRepository {
    suspend fun savePlaybackPosition(mediaId: String, title: String, posterUrl: String?, positionMs: Long, durationMs: Long)
...
class PlayHistoryRepositoryImpl @Inject constructor(
    ...
    override suspend fun savePlaybackPosition(
        ...
        // Règle métier : Si on a regardé moins de 10 secondes, on ne sauvegarde pas (évite le bruit)
        if (positionMs < 10_000) return

        // Règle métier : Si on a regardé plus de 90% du contenu, on considère comme "Vu" (IsFinished)
